...
5. **高风险操作自动化失败** (2026-04-01) ⭐ **今日新增**
   - ❌ 问题：修改 `~/.openclaw/openclaw.json` 时遇到 JSON 解析错误
   - ✅ 规则：**高风险操作自动化前需充分验证可行性**
   - 💡 改进：**提供详细步骤，让用户手动执行或选择是否执行**
   - 📝 记录：已添加到 memory/2026-04-02-security-fix.md

6. **exec_shell 滥用风险** (2026-04-01) ⭐ **今日新增**
   - ❌ 问题：`exec_shell` 可以执行任意 Shell 命令,存在删除系统文件等高风险操作
   - ✅ 规则：**`exec_shell` 定义为高风险操作，必须二次确认**
   - 📝 位置：已添加到 AGENTS.md 高风险章节
   - 🔒 安全：已备份配置文件到 `~/.openclaw/backup/`
